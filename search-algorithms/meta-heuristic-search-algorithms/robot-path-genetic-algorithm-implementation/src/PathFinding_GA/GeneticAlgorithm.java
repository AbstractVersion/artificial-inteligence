...
public class GeneticAlgorithm {
	...
	public Chromosome search() {
		...
		for (int t = 0 ; t < this.maxGeneration ; t++) {
			...
			//New
			if (newPopulation.getFittest().getFitness() == 0) {//New
				Object[] bestPath = currentPop.getFittest().getGenes();//New
				FitnessFunction.evaluate(bestPath);//New
				return currentPop.getFittest();//New
			}//New
